...
* **callout:** markup generated by `<ngb-callout>` was simplified, there is no more intermediate `<div>` element
...
After:
```
<ngb-callout role="callout" class="callout callout-warning">
  Hello there
</ngb-callout>
```
...
Check the self-closing callout demo to know how to achieve the same thing with `NgbCallout`.
